<template>
  <a-row class="j-select-biz-component-box" type="flex" :gutter="8">
    <a-col class="left" :class="{ full: !buttons }">
      <slot name="left">
        <a-select mode="multiple" :placeholder="placeholder" v-model="selectValue" :options="selectOptions" allowClear :disabled="disabled" :open="selectOpen" style="width: 100%" @dropdownVisibleChange="handleDropdownVisibleChange" @click.native="visible = buttons || disabled ? visible : true" />
      </slot>
    </a-col>
    ...
    <j-select-biz-component-modal v-model="selectValue" :visible.sync="visible" v-bind="modalProps" @options="handleOptions" />
  </a-row>
</template>
