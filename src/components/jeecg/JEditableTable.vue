        ...
        <div v-if="actionButton" class="action-button">
          ...
          <template v-if="selectedRowIds.length > 0">
            <a-popconfirm :title="`确定要删除这 ${selectedRowIds.length} 项吗?`" @confirm="handleConfirmDelete">
              <a-button v-if="buttonPermission('batch_delete')" type="primary" icon="minus" :disabled="disabled"
                >删除</a-button
              >
              <span class="gap"></span>
            </a-popconfirm>
                    ...
                    <template v-else-if="col.type === formTypes.list_multi">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        ...
                        <span
                          ...
                          :class="{ disabled: buildProps(row, col).disabled }"
                          @click="handleEditRow(row, col)"
                          >{{ getSelectTranslateText(multiSelectValues[id], row, col) }}
                        </span>
                      </a-tooltip>
                    </template>
                    <!-- select多选 -end -->
