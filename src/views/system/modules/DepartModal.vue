  ...
  >
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        ...
        <a-form-model-item label="机构功能" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <j-dict-select-tag v-model="model.caFunction" dictCode="agency_function" placeholder="请选择机构功能" />
        </a-form-model-item>
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="统一信用代码" prop="caSocialCode">
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        ...
        caSocialCode: [{ validator: this.validateCaSocialCode }],
  ...
  methods: {
    ...
    validateCaSocialCode(rule, value, callback) {
      if (!value || new RegExp(/^([0-9A-HJ-NPQRTUWXY]{2}\d{6}[0-9A-HJ-NPQRTUWXY]{10}|[1-9]\d{14})$/).test(value)) {
        callback()
      } else {
