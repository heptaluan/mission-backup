        ...
        <a-row :gutter="24">
          <a-col :md="6" :sm="10">
            <a-form-item label="任务类名">
              <a-input
                placeholder="请输入任务类名"
                v-model="queryParam.jobClassName"
              ></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="10">
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              <a-menu-item
                ><a @click="executeImmediately(record)"
                  >立即执行</a
                ></a-menu-item
              >
  ...
  methods: {
    ...
    pauseJob: function (record) {
      ...
      this.$confirm({
        ...
        content: '是否暂停选中任务?',
        onOk: function () {
          getAction(that.url.pause, { jobClassName: record.jobClassName }).then(
            res => {
              if (res.success) {
