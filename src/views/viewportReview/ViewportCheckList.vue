...
import { getTaskCheckList } from 'src/api/viewport/index'
import ViewportReviewModal from './modules/ViewportReviewModal.vue'

export default {
  name: 'ViewportCheckList',
  ...
  methods: {
    ...
    handleShowDetail(record) {
      this.$router.push({
        path: `/viewportReviewDetail?id=${record.taskId}&orderId=${record.orderId}&resource=${record.dicomGroupId}&taskId=${record.taskId}&doctorId=${record.taskId}&state=admin`
      })
    },
    loadData() {
      const that = this
