<template>
  <div class="conclusion-result padding-top-bottom-xs">
    <div class="result-table b-flex text-center">
      <div class="result-table-col b-flex flex-col item-number">
        <div class="result-table-item item-header">序号</div>
        <div class="result-table-item">1</div>
        <div class="result-table-item">2</div>
        <div class="result-table-item text-h80">3</div>
      </div>
      <div class="result-table-col b-flex-2 b-flex flex-col">
        <div class="result-table-item item-header">项目</div>
        <div class="result-table-item">蛋白组学检测</div>
        <div class="result-table-item">代谢组学检测</div>
        <div class="result-table-item text-h80 padding-left-right-xs">
          <p>医学影像人工智能阅片分析</p>
        </div>
      </div>
      <div class="result-table-col b-flex-2 b-flex flex-col">
        <div class="result-table-item item-header">结果（综合评分）
        </div>
        <div class="result-table-item text-h160">高风险</div>
      </div>
      <div class="result-table-col b-flex-2 b-flex flex-col">
        <div class="result-table-item item-header">结果（检测评分）</div>
        <div class="result-table-item text-h80">高风险（70.8）</div>
        <div class="result-table-item text-h80">高风险（≥60%）</div>
      </div>
      <div class="result-table-col b-flex-4 b-flex flex-col">
        <div class="result-table-item item-header">参考值（评分范围）</div>
        <div class="result-table-item text-h80">
          <p>低风险：0~50分（不含50分）</p>
          <p>中风险：50~70分（含50分）</p>
          <p>高风险：70~100分（含70分）</p>
        </div>
        <div class="result-table-item text-h80">
          <p>低风险：0%≤恶性概率＜60%；</p>
          <p>高风险：60%≤恶性概率≤100%；</p>
        </div>
      </div>
    </div>
    <div class="result-tips">
      <p>注：</p>
      <p>①　“判分分值”和“恶性概率”评分均为泰莱生物自主研发的评判模型计算获得，能有效进行肺结节良恶性区分。</p>
      <p>②　检测数值与肺结节恶变程度呈正相关，分值越大，表示受检者肺结节恶变的风险越大。</p>
      <p>③　检测结果仅针对当次送检的影像数据及血液样本，仅供临床参考，不作诊断。</p>
    </div>
  </div>
</template>

<script>
  export default {
    name: 'conclusion-result'
  }
</script>
