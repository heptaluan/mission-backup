...
<div class="test-result">
 ...
 <h4 v-html="title"></h4>
  ...
  <p class="text-indent" v-if="this.dicomResult.scrynMaligant >= 60">人工智能阅片分析，该受检者双肺共检出{{dicomResult.total}}个结节（具体信息详见下图），其中{{total}}个结节恶性程度评估为高风险（<span>结节恶性风险为：{{dicomResult.scrynMaligant}} %</span>）。<span v-if="total">具体信息为：</span></p>
  <p class="text-indent" v-if="this.dicomResult.scrynMaligant <60 && dicomResult.total">人工智能阅片分析，该受检者双肺共检出{{dicomResult.total}}个结节（具体信息详见下图），经人工智能肺小结节筛查分析显示：结节恶性程度均为<strong>低风险</strong>。</p>
...
export default {
  ...
  methods: {
    init () {
      ...
      const text = this.$route.params.template === 'sd' ? '生化分子是生物功能的终端产物，其改变可反映生物体某一特定生理时期内的活动情况，与疾病表型关系也最为密切。蛋白大分子物质和代谢小分子物质都属于生物体内的生化分子。' : '生化分子是生物功能的终端产物，其改变可反映生物体某一特定生理时期内的活动情况，与疾病表型关系也最为密切。'
