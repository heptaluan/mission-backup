...
<div class="test-result">
 ...
 <h4 v-html="title"></h4>
  ...
  <p class="text-indent" v-if="this.dicomResult.scrynMaligant >= 70">人工智能阅片分析，该受检者双肺共检出{{dicomResult.total}}个结节（具体信息详见下图），其中{{total}}个结节恶性程度评估为高风险（<span>结节恶性风险为：{{dicomResult.scrynMaligant}} %</span>）。<span v-if="total">具体信息为：</span></p>
  <p class="text-indent" v-if="this.dicomResult.scrynMaligant <70 && dicomResult.total">人工智能阅片分析，该受检者双肺共检出{{dicomResult.total}}个结节（具体信息详见下图），经人工智能肺小结节筛查分析显示：结节恶性程度均为<strong>低风险</strong>。</p>
  <p class="text-indent" v-if="!dicomResult.total">人工智能阅片分析，该受检者双肺未检出结节。</p>
</div>
</template>
...
export default {
  ...
  computed: {
    suggest () {
      ...
      const _suggest = ananpanReportValue >= 70 ? proteinResult.high.suggest : (ananpanReportValue >= 50 ? proteinResult.mid.suggest : '')
      return _suggest
    },
    total () {
    ...
    title () {
      return this.$route.params.template === 'sd' ? '1. 生化分子组学（蛋白组学/代谢组学）检测结果' : '1. 生化分子组学检测结果'
    }
  },
  mounted () {
    this.init()
  ...
  methods: {
    init () {
      ...
      const text = this.$route.params.template === 'sd' ? '生化分子是生物功能的终端产物，其改变可反映生物体某一特定生理时期内的活动情况，与疾病表型关系也最为密切。蛋白大分子物质和代谢小分子物质都属于生物体内的生化分子。' : '化分子是生物功能的终端产物，其改变可反映生物体某一特定生理时期内的活动情况，与疾病表型关系也最为密切。'
      this.result.content = `${text}通过对肺癌发生发展中某些显著性差异的内源性分子物质的含量检测，经综合评估，该样本结节良恶性评分为${this.data.ananpanReportValue}分，肺结节恶性程度为:`
      this.dicomResult = JSON.parse(this.data.imageReportAll)
      this.dicomResult.scrynMaligant = this.data.imageReportValue
      this.dicomResult.interpretation = this.dicomResult.scrynMaligant >= 70 ? dicom.interpretation : dicom.low
