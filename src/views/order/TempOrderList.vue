          ...
          <template v-if='toggleSearchStatus'>
            ...
            <a-col class='group sm'>
              <a-form-item label='状态:' :labelCol='{ span: 6 }'>
                <a-select v-model='queryParam.orderState' placeholder='请选择状态' disabled>
    ...
    <div>
      ...
      >
        ...
        <span slot='action' slot-scope='text, record' style='display: flex;justify-content: space-evenly;'>
          <a v-if='record.orderState === 1000' @click='editOrder(record)'>审核</a>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: 'multiomics/productOrder/orderUserView?queryState=0',
  ...
  methods: {
    ...
    setTemp() {
      this.queryParam.orderState = 1000
      this.loadData(1)
    }
  },
  mounted() {
